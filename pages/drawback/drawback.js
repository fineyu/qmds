...
function afterSales(order_id, reason, type, name, desc, img_url, phone) {
  http.postReq("/order/after-order", { order_id: order_id, reason: reason, type: 1, name: name, desc: desc, img_url: img_url, phone: phone },      function (res) {
    if(res.state){
    }else{
      wx.showToast({
        title: res.msg,
...
Page({
  ...
  submit:function(){
    ...
    if(name==""){
      ...
      afterSales(order_id, reason, 1, name, desc, imgStr, phone);
    wx.navigateTo({
